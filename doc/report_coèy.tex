...
\end{array}
\end{equation}
\textit{Answer.} First, using the solution from Sections~\ref{sec:zero-sum}, we write the dual of the inner maximisation problem using the same logic where we set $\theta$ to a constant: 
...
To simplify finding its dual, we rewrite the linear program as the following: 

\begin{equation}
\begin{array}{lll@{}lll}
&\max\limits_{z_1,...,z_N}  &\frac{1}{k}\displaystyle\sum\limits_{i=1}^{N}z_i|y^{(i)} - \theta^\top x^{(i)}| +  \lambda||\theta||_1 &\\
\text{subject to} 
&& \displaystyle\sum\limits_{i=1}^{N}z_i \leq k  &&  \\
&& -\displaystyle\sum\limits_{i=1}^{N}z_i \leq -k  &&  \\
                \\
...
Then, we write its dual as:

\begin{equation*}
\begin{array}{lll@{}lll}
&\displaystyle\min\limits_{y}   &  k\beta_{N + 1 }  + \displaystyle\sum\limits_{i=1}^{N}\beta_i  + \lambda||\theta||_1  &\\
\text{subject to} 
&& \frac{1}{k}|y^{(i)} -\theta^\top x^{(i)}|  \leq  \beta_{N + 1 } + \beta_i&&  i=1 ,\dots, N 
                \\
                && y_i \geq 0 &&  i=1 ,\dots, N 
\end{array}
\end{equation*}

From this, we change $\beta_{N+1}$ to $\alpha$, and use the fact that $|y_i - \theta^T x_i| \leq \beta_i$ is equivalent to $-\beta_i < y_i - \theta^T x_i  < \beta_i $ and that we can add $\alpha$ to both sides ($-\beta_i + \alpha < y_i - \theta^T x_i  < \beta_i + \alpha$). The same process is applied to $|\theta_i|$ but with an auxiliary variable $b_i$. 
