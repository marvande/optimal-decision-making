...
Upon implementing the modified problem, we get 
...
&&  \beta_{N + 1 } + \beta_i   & \geq \frac{1}{k}|y^{(i)} - \theta^\top x^{(i)}| &  i=1 ,\dots, N \\
                \\
                && y_i    & unconstrained& %% ?? Really unconstrained ? 
\end{array}
\end{equation*}
...
We notice a significant difference between the MEA on the test set against the the Training Set. 
\end{enumerate}
