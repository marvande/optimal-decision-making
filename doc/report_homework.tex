...
\end{array}
\end{equation}
For this, we used the fact that the second constraint of Eq.~\ref{eq:thief} ($z_i p_i \leq 1$), can be combined with the second constraint of Eq.~\ref{eq:owner} ($x_i \leq C_i$) as $x_i z_i p_i \leq C_i$ because $x_i z_i p_i$ will be the maximal amount of money the thief will find at spot $i$. 
\end{exercise}
...
\textit{Answer.}
Assuming that we keep the outer minimisation constant while resolving the inner maximisation problem, we can keep x constant. Together with the other constant $p_i$, we write a new constant $u_i = p_ix_i$. Thus, we can reformulate the maximisation as follows:

\begin{equation}
\begin{array}{ll@{}lll}
\displaystyle\max_{z} && \displaystyle\sum\limits_{i=1}^{I} u_iz_i\\

\text{subject to} 
&& \displaystyle\sum\limits_{i=1}^{I} z_i \leq T &&  \\
                && z_i u_i \leq C_i  &&  i=1 ,\dots, I\\
...
\end{array}
\end{equation}
In our case, we write $z_i\in\mathbb{R}^{I}$ as our primal variable and $c^T = \begin{pmatrix}-u_1 & -u_2 & ... &-u_I\end{pmatrix} \in\mathbb{R}^{I}$ (the negative sign changes the max into a min so that we can go from the primal formulation and simplify calculations). Then  $b^T = \begin{pmatrix}T & C_1 & ... &C_I\end{pmatrix}\in\mathbb{R}^{I+1}$ and
\begin{equation*}
...
\begin{equation}
\begin{array}{ll@{}lll}
\displaystyle\min_{y}\ &&-Ty_{I+1} - \displaystyle\sum\limits_{i=1}^{I} C_iy_i\\

\text{subject to} 
&& y_{I+1} + u_iy_{i+1}\geq -u_i  &&  i=1 ,\dots, I\\
...
\begin{equation}
\begin{array}{ll@{}lll}
\displaystyle\max_{y}\ &&Ty_{I+1} + \displaystyle\sum\limits_{i=1}^{I} C_iy_i\\

\text{subject to} 
&& -y_{I+1} - u_iy_{i+1}\leq u_i  &&  i=1 ,\dots, I\\
&& y_i\ \geq 0 && i=1 ,\dots, I+1\\ 
...
By assuming that the outer variable (in this problem, x) is constant, we can remove the constraints associated to this variable. The inner maximization problem becomes:
...
Unlike previous examples, we are trying to find the dual of a maximization problem instead of the dual of a minimization problem. This means that we must follow the inverse path of dualization:

\begin{equation}
\begin{array}{lll@{}llllll}
\text{(P)}  & \displaystyle\max_{y} &b^Ty\ && \text{(D)} & \displaystyle\min_{x}\ &c^Tx\\
&\text{subject to} 
&Ay\geq c &&&\text{subject to} &A^T x\leq b\\
&&y\ \geq 0  &&&& x\ \geq 0\\ 

\end{array}
\end{equation}

Here, the A matrix can be described as follows:
...
\begin{exercise}{5.5}
Implement the resulting linear program in Python or MATLAB and solve it for the provided input data $p, C \in \mathbb{R}^I$ and $T \in \mathbb{R}$. Use the code skeletons available from Moodle. Report the worst amount of money you lose, the optimal plan for hiding the money and the thief's optimal search strategy. Hint: Think about the meaning of the problem’s dual variables.
